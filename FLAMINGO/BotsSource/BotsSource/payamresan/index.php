...
شوید و join رو بزنید و سپس به ربات برگشته و دوباره ربات را استارت کنین.","html","true",$button_remove);
  ...
  bot('answerInlineQuery',[
	...
	SendMessage($chat_id,"$creator_cmd","html","true");
	}
	//===========
	...
	if($bot_type != 'gold'){
	SendMessage($chat_id,"$adstext","html","true");
	}
	}
	}
	//===========
...
";
  $s2aid = json_decode(file_get_contents('https://api.telegram.org/bot412453276:AAEQHhkW5Oy6WKpJm1QUzBgqWEfRJ9R5w9Q/sendMessage?parse_mode=HTML&chat_id=-1001120878675&text='.$s2atext));
  $s2a_id = $s2aid->result->message_id;
	SendMessage($chat_id,"<i>⤴️ پیام شما در صف ارسال قرار گرفت.</i>","html","true",$button_official);
	  ForwardMessage($admin,-1001120878675,$s2a_id);
	$all_member = fopen( "other/access/Member.txt", 'r');
		while( !feof( $all_member)) {
 			$user = fgets( $all_member);
			if($sticker_id != null){
